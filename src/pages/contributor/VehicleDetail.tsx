...
const VehicleDetail: React.FC<VehicleDetailProps> = ({ onLogout, user }) => {
    ...
    const fetchDetail = async (page: number = 1) => {
        ...
        try {
            let url = `utilisateur/get-contributor-vehicle-details/${id}?user_id=${user.id}&page=${page}`;
            ...
            if (response.data.success) {
                // Ensure fleet matches the expected { vehicle: ... } structure by wrapping it
                setFleet({ vehicle: response.data.data.vehicle });
        ...
        return (
            <ContributorLayout user={user} onLogout={onLogout}>
                <div className="flex flex-col items-center justify-center min-h-[60vh] text-slate-400">
                    ...
                    <Link to="/contributor/vehicles" className="mt-4 px-6 py-2 bg-primary text-white rounded-full font-bold">
                        Retour à la flotte
                    </Link>
                </div>
            </ContributorLayout>
            ...
            <div className="max-w-6xl mx-auto font-body">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-6 md:mb-8 gap-4 md:gap-6">
                    <div className="w-full md:w-auto">
                        <div className="flex items-center flex-wrap gap-2 text-slate-400 text-[10px] md:text-xs mb-3 font-bold uppercase tracking-widest">
                            <Link to="/contributor/dashboard" className="hover:text-primary transition-colors">Dashboard</Link>
                            <span className="material-symbols-outlined text-[10px]">chevron_right</span>
                            <Link to="/contributor/vehicles" className="hover:text-primary transition-colors">Vehicles</Link>
                            <span className="material-symbols-outlined text-[10px]">chevron_right</span>
                            <span className="text-primary truncate max-w-[100px] sm:max-w-none">{v?.matricule}</span>
                        </div>
                        <h2 className="text-3xl md:text-4xl font-black text-[#00327d] tracking-tight uppercase leading-none break-words">
                    ...
                    <div className="flex gap-3 w-full md:w-auto">
                        <Link 
                            to={`/contributor/vehicle-report/${id}`}
                            className="w-full md:w-auto justify-center px-6 py-2.5 rounded-full bg-primary text-white font-bold text-xs uppercase tracking-widest hover:opacity-90 transition-all flex items-center gap-2 shadow-lg shadow-primary/20"
                        >
                ...
                {/* Tabs Navigation */}
                <div className="flex flex-nowrap overflow-x-auto no-scrollbar gap-1 mb-6 md:mb-8 p-1 bg-slate-100 rounded-2xl w-full sm:w-fit">
                    {['chauffeur', 'course', 'vehicle', 'category'].map(tab => (
                        <button 
                            key={tab}
                            onClick={() => setActiveTab(tab)}
                            className={`whitespace-nowrap px-6 md:px-8 py-2 md:py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${
                                activeTab === tab 
                ...
                <div className="w-full">
                    <div className="w-full">
                        <div className="bg-white rounded-[2rem] p-5 md:p-8 shadow-sm ring-1 ring-black/5 min-h-[400px]">
                            {activeTab === 'chauffeur' && (
                                <div className="animate-in fade-in slide-in-from-bottom-4 duration-300">
                                    <div className="flex flex-col sm:flex-row items-center sm:items-start text-center sm:text-left gap-6 md:gap-8 mb-8 md:mb-10">
                                        <div className="w-24 h-24 md:w-32 md:h-32 rounded-3xl bg-blue-50 flex items-center justify-center text-primary text-3xl md:text-4xl font-black shadow-inner">
                                            {d?.photo ? (
                                        ...
                                        <div>
                                            ...
                                            <p className="text-slate-500 font-bold mt-2 uppercase tracking-widest text-[9px] md:text-[11px] bg-slate-50 inline-block px-4 py-1.5 rounded-full border border-slate-100">
                                                Active Fleet Driver
                                            </p>
                            ...
                            {activeTab === 'course' && (
                                <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
                                    ...
                                    {/* Filter Bar */}
                                    <div className="flex flex-wrap gap-3 md:gap-4 p-4 md:p-6 bg-slate-50 rounded-[2rem] border border-slate-100 shadow-sm">
                                        ...
                                        <div className="flex w-full md:w-auto items-end gap-2 mt-2 md:mt-0">
                                            <button 
                                                onClick={() => fetchDetail(1)}
                                                className="flex-1 md:flex-none justify-center px-6 md:px-8 py-3 bg-[#00327d] text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:opacity-90 transition-all shadow-xl shadow-blue-900/20 flex items-center gap-2"
                                            >
                                            ...
                                            courses.map((attr: any) => (
                                                <div key={attr.id} className="p-6 bg-white rounded-[2rem] flex flex-col md:flex-row justify-between items-start md:items-center border border-slate-100 hover:border-primary/20 hover:shadow-xl hover:shadow-primary/5 transition-all group gap-6">
                                                    ...
                                                    <div className="flex flex-row md:flex-col items-center md:items-end justify-between w-full md:w-auto gap-4 border-t md:border-t-0 pt-4 md:pt-0 border-slate-50">
                                                        ...
                                                        <Link 
                                                            to={`/contributor/rides/${attr.courses?.id}`}
