            ...
            <div className="max-w-6xl mx-auto font-body">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 gap-6">
                    <div className="w-full md:w-auto">
                        <div className="flex flex-wrap items-center gap-2 text-slate-400 text-[10px] md:text-xs mb-3 font-bold uppercase tracking-widest">
                            <Link to="/partner/dashboard" className="hover:text-primary transition-colors">Dashboard</Link>
                            <span className="material-symbols-outlined text-[10px]">chevron_right</span>
                            <Link to="/partner/vehicles" className="hover:text-primary transition-colors whitespace-nowrap">Vehicles</Link>
                            <span className="material-symbols-outlined text-[10px]">chevron_right</span>
                            <span className="text-primary">{v?.matricule}</span>
                        </div>
                        <h2 className="text-3xl md:text-4xl font-black text-[#00327d] tracking-tight uppercase leading-none mt-1">
                    ...
                    <div className="flex gap-3 w-full md:w-auto mt-2 md:mt-0">
                        <Link 
                            ...
                            className="w-full md:w-auto justify-center px-6 py-3 md:py-2.5 rounded-full bg-primary text-white font-bold text-xs uppercase tracking-widest hover:opacity-90 transition-all flex items-center gap-2 shadow-lg shadow-primary/20"
                        >
                ...
                {/* Tabs Navigation */}
                <div className="flex gap-1 mb-8 p-1 bg-slate-100 rounded-2xl w-full md:w-fit overflow-x-auto no-scrollbar scroll-smooth">
                    {['chauffeur', 'course', 'vehicle', 'category'].map(tab => (
                        <button 
                            key={tab}
                            onClick={() => setActiveTab(tab)}
                            className={`px-6 md:px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap flex-shrink-0 ${
                                activeTab === tab 
                ...
                <div className="w-full">
                    <div className="w-full">
                        <div className="bg-white rounded-3xl p-6 md:p-8 shadow-sm ring-1 ring-black/5 min-h-[400px]">
                            {activeTab === 'chauffeur' && (
                                <div className="animate-in fade-in slide-in-from-bottom-4 duration-300">
                                    <div className="flex flex-col md:flex-row items-center md:items-start text-center md:text-left gap-6 md:gap-8 mb-10">
                                        <div className="w-24 h-24 md:w-32 md:h-32 rounded-3xl bg-blue-50 flex-shrink-0 flex items-center justify-center text-primary text-3xl md:text-4xl font-black shadow-inner">
                                            {d?.photo ? (
                                        ...
                                        <div>
                                            <h3 className="text-2xl md:text-3xl font-black text-[#00327d]">{d?.nom} {d?.prenom}</h3>
                                            <p className="text-slate-500 font-bold mt-2 uppercase tracking-widest text-[10px] md:text-[11px] bg-slate-50 inline-block px-4 py-1.5 rounded-full border border-slate-100">
                                                Active Fleet Driver
                                            </p>
                                            <div className="flex flex-wrap justify-center md:justify-start gap-4 md:gap-8 mt-6">
                                                <div className="flex items-center gap-3 bg-slate-50 md:bg-transparent px-4 py-3 md:p-0 rounded-2xl w-full sm:w-auto">
                                                    <div className="w-10 h-10 rounded-full bg-white md:bg-slate-50 flex items-center justify-center text-slate-400 shadow-sm md:shadow-none">
                                                ...
                                                <div className="flex items-center gap-3 bg-slate-50 md:bg-transparent px-4 py-3 md:p-0 rounded-2xl w-full sm:w-auto">
                                                    ...
                                                    <div className="text-left">
                                                        ...
                                                        <p className="font-bold text-slate-700 truncate max-w-[150px] md:max-w-none">{d?.email || 'N/A'}</p>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                            ...
                            {activeTab === 'course' && (
                                <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
                                    ...
                                    {/* Filter Bar */}
                                    <div className="flex flex-col md:flex-row flex-wrap gap-4 p-4 md:p-6 bg-slate-50 rounded-[2rem] border border-slate-100 shadow-sm">
                                        ...
                                        <div className="flex flex-row gap-4 flex-1 w-full md:w-auto">
                                            <div className="flex-1 min-w-[120px] md:min-w-[150px]">
                                                ...
                                                <input 
                                                    ...
                                                    className="w-full bg-white border border-slate-200 rounded-2xl px-3 md:px-4 py-3 text-xs font-black text-slate-700 focus:ring-4 focus:ring-primary/5 outline-none transition-all"
                                                />
                                            </div>
                                            <div className="flex-1 min-w-[120px] md:min-w-[150px]">
                                        ...
                                        <div className="flex items-end gap-2 w-full md:w-auto pt-2 md:pt-0 border-t md:border-none border-slate-200 mt-2 md:mt-0">
                                            <button 
                                                onClick={() => fetchDetail(1)}
                                                className="flex-1 md:flex-none px-6 md:px-8 py-3 bg-[#00327d] text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:opacity-90 transition-all shadow-md md:shadow-xl shadow-blue-900/20 flex items-center justify-center gap-2"
                                            >
                                            ...
                                            <button 
                                                ...
                                                className="px-4 py-3 bg-white border border-slate-200 text-slate-400 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-100 transition-all flex items-center justify-center"
                                            >
                                    ...
                                    <div className="grid grid-cols-1 gap-4">
                                        {courses.length > 0 ? (
                                            courses.map((attr: any) => (
                                                <div key={attr.id} className="p-5 md:p-6 bg-white rounded-[2rem] flex flex-col md:flex-row justify-between items-start md:items-center border border-slate-100 hover:border-primary/20 hover:shadow-xl hover:shadow-primary/5 transition-all group gap-4 md:gap-6">
                                                    <div className="flex items-center gap-4 md:gap-5 flex-1 w-full">
                                                        <div className="w-12 h-12 md:w-14 md:h-14 rounded-2xl bg-slate-50 flex items-center justify-center text-primary group-hover:bg-primary group-hover:text-white transition-all shadow-sm shrink-0">
                                                            <span className="material-symbols-outlined text-xl md:text-2xl">route</span>
                                                        </div>
                                                        <div className="flex-1 min-w-0">
                                                            <div className="flex items-center gap-2 md:gap-3 mb-1">
                                                                <p className="text-xs md:text-sm font-black text-slate-800 uppercase tracking-tight truncate">Trip #{attr.courses?.id}</p>
                                                                <span className={`shrink-0 text-[7px] md:text-[8px] font-black uppercase px-2 py-0.5 rounded-md ${
                                                                    attr.courses?.statut === 'TERMINEE' ? 'bg-emerald-100 text-emerald-700' : 'bg-blue-100 text-blue-700'
                                                                }`}>{attr.courses?.statut}</span>
                                                            </div>
                                                            <div className="flex flex-col gap-1">
                                                                <div className="flex items-center gap-1.5 md:gap-2 text-[9px] md:text-[10px] text-slate-500 font-bold group-hover:text-primary transition-colors">
                                                                    <span className="material-symbols-outlined text-[13px] md:text-[14px]">calendar_today</span>
                                                                    {new Date(attr.created_at).toLocaleDateString('fr-FR', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' })}
                                                                </div>
                                                                <div className="flex items-center gap-1.5 md:gap-2 text-[9px] md:text-[10px] text-slate-400 font-medium truncate italic max-w-full md:max-w-md">
                                                                    <span className="material-symbols-outlined text-[13px] md:text-[14px] shrink-0">location_on</span>
                                                                    <span className="truncate">{attr.courses?.lieu_depart} → {attr.courses?.lieu_arrive}</span>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div className="flex flex-row md:flex-col items-center justify-between w-full md:w-auto gap-4 border-t md:border-t-0 pt-4 md:pt-0 border-slate-100 mt-2 md:mt-0">
                                                        <div className="text-left md:text-right">
                                                            <p className="text-base md:text-lg font-black text-primary leading-none">{attr.courses?.montant?.toLocaleString()} <span className="text-[9px] md:text-[10px] opacity-60 text-slate-400 font-bold ml-0.5 md:ml-1">XAF</span></p>
                                                        </div>
                                                        <Link 
                                                            to={`/partner/courses/${attr.courses?.id}`}
                                                            className="px-5 md:px-6 py-2 md:py-2.5 bg-slate-50 text-slate-600 rounded-xl md:rounded-2xl text-[8px] md:text-[9px] font-black uppercase tracking-widest hover:bg-primary hover:text-white transition-all flex items-center gap-1 md:gap-2 border border-slate-100"
                                                        >
                                                            Détails
                                                            <span className="material-symbols-outlined text-[12px] md:text-sm">arrow_forward</span>
                                                        </Link>
                                                    </div>
                                                </div>
                                            ))
                                    ...
                                    {coursePagination && coursePagination.last_page > 1 && (
                                        <div className="flex flex-col sm:flex-row justify-between items-center mt-8 md:mt-10 px-6 py-4 bg-slate-50 rounded-2xl border border-slate-100 font-body gap-4 sm:gap-0">
                                            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest w-full sm:w-auto text-center sm:text-left">
                                                Page {coursePagination.current_page} of {coursePagination.last_page}
                                            </span>
                                            <div className="flex gap-1 md:gap-2 flex-wrap items-center justify-center">
                                                {coursePagination.links.map((link: any, index: number) => {
                                                    // On mobile, only show prev, next, first, last, and current active link to save space
                                                    ...
                                                    const showOnMobile = isArrow || isActive;
                                                    ...
                                                    return (
                                                        <button
                                                            ...
                                                            className={`px-3 md:px-4 py-2 rounded-xl font-black transition-all text-[9px] uppercase tracking-widest ${!showOnMobile ? 'hidden sm:inline-block' : ''} ${
                                                                link.active 
