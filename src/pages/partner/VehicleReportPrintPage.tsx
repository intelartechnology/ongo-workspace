...
const VehicleReportPrintPage: React.FC = () => {
    ...
    useEffect(() => {
        const fetchReportData = async () => {
            try {
                ...
                const res = await api.getData(`utilisateur/get-partner-vehicle-details/${id}`);
                if (res.data.success) {
    ...
    const vehicle = data.fleet?.vehicle;
    const history = (data.courses?.data || []).filter((c: any) => c.courses?.statut === 'TERMINEE');
            ...
            <div className="max-w-[21cm] mx-auto bg-white shadow-2xl p-[1.5cm] min-h-[29.7cm] border border-slate-100 print:shadow-none print:border-none print:p-0 relative overflow-hidden">
                ...
                <div className="grid grid-cols-4 gap-4 mb-12">
                   {[
                       ...
                       { label: 'Revenu Est.', val: history.filter((c:any)=>c.courses?.statut === 'TERMINEE').reduce((acc:number, curr:any)=> acc + (curr.courses?.montant * 0.8), 0).toLocaleString() + ' F', icon: 'payments', scheme: 'indigo' },
                   ].map((stat, i) => (
                ...
                <div className="mt-auto pt-10 border-t border-dashed border-slate-200">
                    <div className="flex justify-between items-end">
                        <div className="max-w-[400px]">
                            <p className="text-[9px] font-bold text-slate-400 italic">Ce document constitue un rapport officiel généré automatiquement par la plateforme ON GO. Les montants affichés incluent les commissions partenaires de 80% le cas échéant.</p>
